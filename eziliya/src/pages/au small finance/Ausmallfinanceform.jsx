...
export default function Ausmallfinanceform() {
      ...
      if (formId) {
        try {
          ...
          if (response.data.success && response.data.data) {
            ...
            // The backend returns the entire document, use it directly
            const loadedData = response.data.data;
            console.log('Loaded form data:', loadedData); // Debug log
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Extract required fields with better fallbacks
      const applicantName = formData.ApplicantsNames || formData.applicantsName || formData.NameofCurrentOwnerSeller || 'Application';
      const propertyAddress = formData.AddressAsperTRF || formData.propertyAddress || formData.AddressasperActualAtsite || 'Address';
      const loanAmount = parseFloat(formData.loanAmount) || parseFloat(formData.LoanAmount) || 0;
      
      let formIdToSubmit = savedFormId;
      
      // If we don't have a saved form, create one first
      if (!formIdToSubmit) {
        ...
        const createResponse = await axios.post(
          `${API_BASE_URL}/ausmall-finance-form/create`,
          {
            ...
            loanAmount,
            propertyAddress,
            ...formData // Spread all form data to ensure all fields are saved
          },
          { headers: { 'Authorization': `Bearer ${token}` } }
        );
        
        if (!createResponse.data.success) {
          throw new Error(createResponse.data.message || 'Failed to create form');
        }
        
        formIdToSubmit = createResponse.data.data._id;
        console.log('Form created with ID:', formIdToSubmit);
      }
      
      // Now submit the form
      console.log('Submitting form with ID:', formIdToSubmit);
      const submitResponse = await axios.post(
        `${API_BASE_URL}/ausmall-finance-form/${formIdToSubmit}/submit`,
        {
          action: 'submit',
      ...
      if (!submitResponse.data.success) {
        throw new Error(submitResponse.data.message || 'Failed to submit form');
      }
      ...
      console.log('Number of fields in payload:', Object.keys(reportPayload).length);
      
      alert('Form submitted successfully! Navigating to final report...');
      ...
      <form className={styles.form} onSubmit={handleSubmit}>
        ...
        <div>
          <label htmlFor="Totalvalue">
            Total Construction Value for 100%complete building (in
            Rs)  :
          </label><br></br>
          <input
            type="text"
        ...
        {formData.typeOfProperty &&
         formData.typeOfProperty !== 'Bungalow' &&
         formData.typeOfProperty !== 'Row House' && (
          <div>
