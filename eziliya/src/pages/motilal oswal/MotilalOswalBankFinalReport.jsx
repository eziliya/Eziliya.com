...
import styles from './MotilalOswalBankFinalReport.module.css'
import html2canvas from 'html2canvas'
import { jsPDF } from 'jspdf'

export default function MotilalOswalBankFinalReport() {
  ...
  const handleDownloadPdf = async () => {
    ...
    try {
      ...
      const canvas = await html2canvas(element, {
        ...
        onclone: (doc) => {
          const cloned = doc.querySelector('[data-pdf-root="motilal"]')
          if (cloned) { cloned.classList.add(styles.pdfMode); cloned.style.backgroundColor = '#ffffff'; cloned.style.color = '#000000' }
        },
      })
      if (!canvas.width || !canvas.height) { toast.error('Could not capture the report'); return }
  ...
  const handleChange = (e) => {
    ...
    if (!name) return;
    if (type === 'file') { setFormData((prev) => ({ ...prev, [name]: files?.[0] || null })); return; }
  ...
  const branchNameId=formData.branchNameId||'';const remark1=formData.remark1||'';const remark2=formData.remark2||'';const remark3=formData.remark3||'';
  const remark4=formData.remark4||'';const remark5=formData.remark5||'';
  ...
  return (
    <div className={styles.container}>
      ...
      <div ref={reportRef} data-pdf-root="motilal" onChange={handleChange}>
        <table>
