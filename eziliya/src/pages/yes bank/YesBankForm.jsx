...
import styles from './YesBankForm.module.css';
...
export default function YesBankForm() {
  const [formData, setFormData] = useState({});
  const [isUploading, setIsUploading] = useState(false);
  const navigate = useNavigate();
  const location = useLocation();
  const reportId = location.state?.reportId; // Get reportId if editing existing report
  ...
  const handleSubmit = async (e) => {
    ...
    // First navigate to report preview (original behavior)
    navigate('/YesBank-report', { state: { formData: reportPayload } });
    
    // Then try to submit to workflow in background
    setIsUploading(true);
    try {
      const token = localStorage.getItem('token');
      
      if (token) {
        // Create or update report with workflow integration
        let currentReportId = reportId;
        
        if (!currentReportId) {
          // Create new report first - assigned to Office Engineer (technician)
          const createResponse = await axios.post(
            `${API_BASE_URL}/create`,
            {
              taskTitle: formData.NameofApplicant || 'Yes Bank Report',
              propertyType: formData.PropertyType || 'other',
              address: formData.PropertyAddress || 'To be confirmed',
              assignedTo: 'technician' // Office Engineer role
            },
            {
              headers: {
          ...
          currentReportId = createResponse.data.report._id;
        }

        // Submit Office Engineer data to workflow
        await axios.post(
          `${API_BASE_URL}/submit-office-engineer/${currentReportId}`,
          {
            bankName: 'Yes Bank',
        ...
        console.log('Report submitted to workflow successfully');
      }
    } catch (error) {
      console.error('Workflow submission error (non-blocking):', error);
      // Don't show error to user since report preview already loaded
    } finally {
      setIsUploading(false);
    }
  };

  // Extract individual values from formData for easier access
  ...
  return (
      <div className={styles.container}>
        <form className={styles.form} onSubmit={handleSubmit}>
          ...
          <input type='text' name='dateOfSiteVisit' id="dateOfSiteVisit" className={styles.input} value={dateOfSiteVisit} onChange={handleChange}></input><br></br>
