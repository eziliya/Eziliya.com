...
import styles from './SundaramForm.module.css';
...
function buildReportPayload(data) {
  ...
  copyIf('LocalityzoningtypeasperLatestDevelopmentMasterPlan', 'Locality/zoning typeasperLatestDevelopmentMasterPlan');
  copyIf('Total-Loading%onCarpetArea+BalconyArea+DryBalconey', 'Total-Loading%onCarpetArea+BalconyArea+DryBalconey');
  const fileToReport = {
    satelliteMap: 'Uploadphotosatelitemap', hall: 'UploadphotoHall', kitchen: 'UploadphotoKichen',
    bedroom: 'UploadphotoBedroom', otherRoom: 'UploadphotoOtherRoom', otherPhoto: 'UploadphotoOtherPhoto',
    externalPhoto: 'UploadphotoExternalPhoto', frontSite: 'UploadphotoFrontSite', roadSite: 'UploadphotoRoadSite',
    selfieWithProperty: 'UploadphotoSelfieWithProperty', selfieWithPerson: 'UploadphotoSelfieWithPerson',
  };
  for (const [from, to] of Object.entries(fileToReport)) { copyIf(from, to); }
  return out;
}

export default function SundaramForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    navigate('/Sundaram-report', { state: { formData: reportPayload } });
    setIsUploading(true);
    try {
